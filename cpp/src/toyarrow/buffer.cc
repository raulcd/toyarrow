...
Buffer::Buffer(int64_t size) : m_size(size) {
    ...
    m_data = static_cast<uint8_t*>(::operator new(size, std::align_val_t{kDefaultAlignment}));
    // Use a custom deleter to ensure proper deallocation with alignment.
    owner_ = std::shared_ptr<void>(m_data, [](void* p) { operator delete(p, std::align_val_t{kDefaultAlignment}); });
}
